...
export default async function DashboardLayout({children}:{children:React.ReactNode}){
  ...
  return <div className="admin-shell"><aside className="admin-sidebar"><div className="admin-brand"><span><Landmark size={21}/></span><div><strong>Margo Mulyo</strong><small>Panel Administrator</small></div></div><AdminNavigation/><div className="admin-user"><span>{(admin.full_name||email||'A').charAt(0).toUpperCase()}</span><div><strong>{admin.full_name||'Administrator'}</strong><small>{email}</small></div><form action={logout}><button aria-label="Keluar"><LogOut size={17}/></button></form></div></aside><div className="admin-content">{children}</div></div>
}
